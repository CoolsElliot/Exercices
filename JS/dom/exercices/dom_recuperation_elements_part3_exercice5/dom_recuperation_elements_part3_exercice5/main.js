...
let allH2 = document.querySelector("#liste-soft-skills").children;
...
// odre des index de allH2 nécéssaire dans la boucle : 1, 2, 0, 3

let i = 1; 
let count = 0; 


for(const test in softSkills){
    if(count === 2){
        i = 0; 
    }else if(count === 3 ){
        i = 3;
    };
    allH2[i].innerText = `${test} : ${softSkills[test]}`;
    i++;
    count++;  
};
